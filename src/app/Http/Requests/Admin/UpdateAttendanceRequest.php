...
class UpdateAttendanceRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($v) {
            // ルートの {attendance} モデルを取得（/admin/attendance/{attendance}）
            $attendance = $this->route('attendance');

            // 基準日を安全に決定（nullableなdateに対応）
            $baseDateStr =
                $this->input('date')
                ?? optional($attendance->work_date)->toDateString()
                ?? optional($attendance->clock_in)->toDateString()
                ?? now()->toDateString();

            // createFromFormatではなくparseで安全に
            $date = \Illuminate\Support\Carbon::parse($baseDateStr, 'Asia/Tokyo');
