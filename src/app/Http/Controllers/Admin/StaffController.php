...
class StaffController extends Controller
{
    ...
    public function show(Request $request, User $staff)
    {
        ...
        foreach (CarbonPeriod::create($start, $end) as $day) {
            ...
            // ★ 実勤務があるか（出勤 or 退勤 or 休憩が1件以上）
            $hasAttendance = $rec && (
                $rec->clock_in ||
                $rec->clock_out ||
                ($rec->breakTimes && $rec->breakTimes->isNotEmpty())
            );
            ...
            $rows[] = [
                'day'       => $day->copy(),
                'rec'       => $rec,
                'break_str' => $hasAttendance ? sprintf('%d:%02d', intdiv($breakMin, 60), $breakMin % 60) : null,
