...
Route::middleware('auth')->group(function () {
    ...
    Route::post('/email/verification-notification', function (Request $request) {
        ...
        // 直前ページへPRGリダイレクト（Refererが無ければ verify へ）
        return back(303, [], route('verification.notice'));
...
Route::middleware(['auth', 'admin'])->prefix('admin')->group(function () {
    ...
    Route::get('/attendance/detail/by-date/{staff}/{date}', function (User $staff, string $date) {
        ...
        // ★ ここを修正：クエリを引き継ぐ + date も明示で渡す
        $params = array_merge(
            request()->only(['return', 'staff', 'month']),
            ['date' => $day]
        );

        return redirect()->route('admin.attendance.show', array_merge([
            'attendance' => $att->id,
        ], $params));
    })->whereNumber('staff')
    ->where('date', '^\d{4}-\d{2}-\d{2}$')
